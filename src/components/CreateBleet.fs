...
type State = { Display: bool; IsModal: bool; Profile: Profile option; Bleet: Bleet option; BleetContent: string}
...
let init() = { Display = false; IsModal = false; Profile = None; Bleet = None; BleetContent = "" }
...
let update (msg:Msg) (state:State) : State =
    ...
    | OutsideModalClickClose id -> if (state.Display && id = "CreateBleetModal" && state.BleetContent.Length = 0) then init() else state
...
let mainElem (state:State) (dispatch: Msg -> unit) =
    ...
    let modalClasses = [
        ...
        tw.``justify-center``  
        tw.``items-center``         
        tw.``bg-black``; tw.``bg-opacity-75``
    ]

    let displayModalClass = 
        if state.Display then [ tw.block ]
        else [ tw.hidden ]

    let classes = 
        [ modalClasses; displayModalClass ] |> List.concat 
        ...
        prop.children [
            Html.div [
                prop.classes [
                    ...
                    tw.``overflow-y-auto``
                    tw.``text-left``
                    tw.``py-4``; tw.``px-6``; tw.``w-2/4``; tw.``h-48``
                    tw.``flex``; tw.``flex-col``
                ]
                prop.children [
                    Html.button [
                    ...
                    Html.div [
                        ...
                        prop.children [
                            Html.img [
                                prop.classes [
                                    tw.``flex``; tw.``flex-grow-0``; tw.``mr-2``
                                    tw.``h-8``; tw.``w-8``
                                    tw.``rounded-full``
                                    tw.``border-2``; tw.``border-gray-100``
                                ]
                                prop.src (profile |> Option.fold (fun _ p -> p.ProfilePic) "")
                            ]
                            Html.div [
                                prop.classes [
                                    tw.flex; tw.``flex-grow-1``; tw.``flex-col``
                                ]
                                prop.children [
                                    Html.textarea [
                                        prop.id "create-bleet-text"
                                        prop.classes [
                                            tw.``resize-none``; tw.``outline-none``
                                        ]
                                        prop.rows 3
                                    ...
                                    Html.div [
                                        ...
                                        prop.children [
                                            Html.button [
                                                ...
                                                prop.classes [
                                                    tw.``bg-green-500``; tw.``text-white``; tw.``rounded-md``
                                                    tw.``px-8``; tw.``py-2``; tw.``w-24``; tw.``float-right``; 
                                                    tw.``text-base``; tw.``font-medium``
                                                    tw.``hover:bg-green-600``
                                                    tw.``focus:outline-none``; tw.``focus:ring-2``; tw.``focus:ring-green-300``
                                                ]
